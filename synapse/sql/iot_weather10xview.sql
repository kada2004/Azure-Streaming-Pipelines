...
(
    SELECT
        DATEADD(minute,(DATEDIFF(minute,0,CAST(CAST(JSON_VALUE(raw_json,'$.payload.measurement_time') AS datetimeoffset) AS datetime2))/10)*10,0)
            AS bucket_10min,
...
(
    SELECT
        DATEADD(minute,(DATEDIFF(minute,0,CAST(CAST(JSON_VALUE(raw_json,'$.event_time') AS datetimeoffset) AS datetime2))/10)*10,0)
            AS bucket_10min,
        ...
        CAST(JSON_VALUE(raw_json, '$.payload.main.temp') AS decimal(6,2)) - 273.15
